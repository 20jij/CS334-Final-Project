...
def hyperparameter_tuning(model, param_grid, X_train, y_train):
    grid_search = RandomizedSearchCV(model, param_distributions=param_grid, n_iter=10, scoring='neg_mean_absolute_error', cv=5, n_jobs=-1, random_state=42)
...
def train_and_evaluate_models(X_train, X_test, y_train, y_test, stock_prices):
    ...
    models = {
        ...
        'Random Forest Regressor': hyperparameter_tuning(RandomForestRegressor(), rf_param_grid, X_train, y_train),
        'Bagging Regressor': hyperparameter_tuning(BaggingRegressor(), bagging_param_grid, X_train, y_train),
        'AdaBoost Regressor': hyperparameter_tuning(AdaBoostRegressor(), adaboost_param_grid, X_train, y_train),
        'Stacking Regressor': hyperparameter_tuning(StackingRegressor(estimators=list(stacking_base_estimators.items()), final_estimator=Ridge()), stacking_param_grid, X_train, y_train),
        'Ridge Regression': Ridge(),
    }

    results_df = pd.DataFrame(columns=['Model', 'MAE', 'MSE', 'RMSE'])
    predictions_df = pd.DataFrame(index=stock_prices.index)
